...
int main(int argc, char *argv[])
{
	...
	int product[BUFFER_SIZE] = { -1,-1,-1,-1,-1 };	// buffer
	int num_pro = 10;											// the number of producers in need 
	int num_con = 10;											// the number of consumers in need
	int time_thr = 60;											// the running time of threads
	int n_pro = 0;												// the counter of producers 
	int n_con = 0;												// the counter of consumers
	HANDLE pro[11];											// the array of producers
	HANDLE con[11];											// the array of consumers8
